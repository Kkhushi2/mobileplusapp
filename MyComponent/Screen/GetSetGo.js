...
export default function GetSetGo() {
    ...
    if (result) {
     ...
     // setTopPick(result);
      ...
      var re= result.filter((item)=>{
        return (item.offerprice < 10000 && item.offerprice > 5000)
      })
      setTopPick(re)
             // setSmlrProduct(re);
    } else {
    ...
    >
      <View
        ...
        <View style={{alignItems: 'flex-start',paddingLeft:10}}>
          ...
          &#x20b9;<Text style={{fontFamily:'Lato-BoldItalic',fontFamily:'Lato-BoldItalic',textDecorationLine:'line-through'}}>{item.price}</Text>
          </Text>
